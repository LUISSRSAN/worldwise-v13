...
return {...state,isLoading:false,
  ...
  case 'cities/deleted': 
  return {...state,isLoading:false,
    cities:state.ities.filter(city=> city.id !==action.payload),
      currentCity: {},
      };
...
dispatchEvent({type:'cities/loaded', payload:data});
}
catch {
...
async function getCity(id){
  ...
  dispatch({type:'loading'})
        ...
        dispatch({typer:'city/loaded',payload:data});
        }
        catch {
  ...
  async function deleteCity(newCity){
    dispatch({type:'loading'})
    try { 
      setIsLoading(true);
    const res = await fetch('${BASE_URL}/cities/${id}',{
